#!/usr/bin/env python3
"""Dry-run or atomically apply the archived R3 mapping clarification."""
...
class ClarificationToolError(ValueError):
    pass
...
def _read(relative: str, maximum: int = 1_048_576) -> bytes:
    ...
    if path.is_symlink() or not path.is_file():
        raise ClarificationToolError(f"input:{relative}")
    raw = path.read_bytes()
    if not 1 <= len(raw) <= maximum:
        raise ClarificationToolError(f"input-size:{relative}")
    return raw


def _rust_route_receipt(route_projection: m2_route_data.R3RouteOwnerProjection) -> bytes:
    projection = canonical_manifest.validate_canonical_manifest(
        route_projection.reproduction_projection
    )
    return canonical_manifest.serialize_manifest(
        {
            "implementation_id": "rust",
            "recipient_package_sha256": projection[
                "recipient_package_sha256"
            ],
            "reproduction_projection_sha256": sha256(
                route_projection.reproduction_projection
            ).hexdigest(),
            "route_data_template_sha256": projection[
...
def build_clarification_projection() -> tuple[
    ...
    """Independently derive all Python-side bytes needed by the atomic gate."""
    ...
    semantic = canonical_manifest.validate_canonical_manifest(
        m2_carrier.render_semantic_envelope(
            compiled, inputs, envelope
        )
    )
    ...
    rust_route_receipt = _rust_route_receipt(projection)
    route_raw = m2_route_data.render_r3_route_data_owner(
        projection,
        projection.python_reproduction_receipt,
        rust_route_receipt,
    )
    if route_raw != tracked_route_raw:
        raise ClarificationToolError("route-drift")
    ...
    archived = m2_policy.validate_r3_pre_clarification_archive(
        ROOT, damage_raw
    )
    promotion_raw = m2_policy.render_r3_mapping_clarification_manifest(
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help="exact independently emitted Rust clarification stage",
    )
    parser.add_argument(
        "--apply",
        action="store_true",
        help="atomically write the three clarified owners; default is dry-run",
    )
    ...
    result = m2_policy.apply_r3_mapping_clarification_bundle(
        ROOT,
        bundle,
        bootstrap_raw,
        fixture_raw,
    ...
    print(f"m2 R3 mapping clarification: {result}")
    return 0
