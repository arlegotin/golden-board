#!/usr/bin/env python3
"""Dry-run or atomically apply the R3 gate-6 convergence refreeze."""
...
class Gate6ConvergenceToolError(ValueError):
    pass
...
def _read(relative: str, maximum: int = 1_048_576) -> bytes:
    ...
    if path.is_symlink() or not path.is_file() or path.stat().st_nlink != 1:
        raise Gate6ConvergenceToolError(f"input:{relative}")
    raw = path.read_bytes()
    if not 1 <= len(raw) <= maximum or len(raw) != path.stat().st_size:
        raise Gate6ConvergenceToolError(f"input-size:{relative}")
    return raw
...
def build_gate6_convergence_projection() -> tuple[
    ...
    if route_raw != tracked_route_raw:
        raise Gate6ConvergenceToolError("route-drift")
    ...
    promotion_raw = m2_policy.render_r3_gate6_convergence_clarification_manifest(
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help="exact independently emitted Rust gate-6 convergence stage",
    )
    parser.add_argument(
        "--apply",
        action="store_true",
    ...
    (
        ...
        rust_limits_receipt,
        comparison_files,
    ) = build_gate6_convergence_projection()
    result = m2_policy.apply_r3_gate6_convergence_clarification_bundle(
        ROOT,
        bundle,
        bootstrap_raw,
        fixture_raw,
    ...
    print(f"m2 R3 gate-6 convergence clarification: {result}")
    return 0
