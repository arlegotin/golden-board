#!/usr/bin/env python3
"""Dry-run or atomically apply the R3 independence-witness refreeze."""
...
class IndependenceWitnessToolError(ValueError):
    pass
...
def _read(relative: str, maximum: int = 1_048_576) -> bytes:
    ...
    if path.is_symlink() or not path.is_file() or path.stat().st_nlink != 1:
        raise IndependenceWitnessToolError(f"input:{relative}")
    raw = path.read_bytes()
    if not 1 <= len(raw) <= maximum or len(raw) != path.stat().st_size:
        raise IndependenceWitnessToolError(f"input-size:{relative}")
    return raw
...
def build_independence_witness_projection() -> tuple[
    ...
    if route_raw != tracked_route_raw:
        raise IndependenceWitnessToolError("route-drift")
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help="exact independently emitted Rust witness-refreeze stage",
    )
    parser.add_argument(
        "--apply",
        action="store_true",
    ...
    (
        ...
        rust_limits_receipt,
        comparison_files,
    ) = build_independence_witness_projection()
    result = m2_policy.apply_r3_independence_witness_clarification_bundle(
        ROOT,
        bundle,
        bootstrap_raw,
        fixture_raw,
    ...
    print(f"m2 R3 independence-witness clarification: {result}")
    return 0
