...
from . import m2_mapping_v2 as mapping, recipe_wire_v2
...
def _first_six():
    ...
    three += bytes((len(scalar_values),)) + b''.join(
        _ints((n,),8)+bytes((len(recipe_wire_v2._uleb(n)),))
        +recipe_wire_v2._uleb(n).ljust(10,b'\0') for n in scalar_values)
    ...
    compact_nodes = sum(1+u(n.output_width)+sum(u(a) for a in n.arguments)
    ...
    observed = (109,32,len(recipe.inputs),len(recipe.outputs),2,
                descriptors,len(recipe.nodes),compact_nodes,32+descriptors+compact_nodes)
    ...
    for op in range(1,26):
        ...
        aux, imm = (3 if op in (2,5,22) else 0), (10 if op in (1,5,14,22,25) else 0)
        six.extend(bytes((op,arity,3*arity,aux,imm,6+3*arity+aux+imm)))
    ...
    for op,kind in ((1,0),(1,1),(3,2),(3,3),(2,4),(24,5)):
        six.extend(bytes((op,kind,(kind<<5)|op)))
...
def _ten(common, encoded):
    package = recipe_wire_v2.decode_recipe_package_v2(build_teaching_recipe_package(),8)
    table = next(t for t in package.logical.tables if t.table_id == 24)
    _check(table.payload == b''.join(encoded), 'group.source-encoding')
    result = bytearray(_ints((123,124,120,125,126,127,24,25,26,27))+bytes((8,57)))
    for case in range(8):
        physical = 11 if case == 7 else 6
        inputs = (physical.to_bytes(4,'big'),bytes((case,)))
        # Source validation executes an exact structural refinement of the
        # same carried program. Generic VM differential checks are separate.
        actual = evaluate_recovery_native(package,126,inputs)
        _check(actual.status == 0 and len(actual.outputs) == 6, 'group.composed-example')
        output = actual.status.to_bytes(2,'big')+b''.join(actual.outputs)
        _check(len(output) == 52, 'group.composed-size')
        result.extend(b''.join(inputs)+output)
    _check(len(result) == 478,'group.definition-size')
    return bytes(result)
