...
The following is the temporary drafting seed for `spec/identity-v0.md`. Before
either implementation is written, M0 promotes this content into that file and,
in the same change, replaces Sections 7.1–7.7 here with a short link and the
acceptance topics only (framing, domain, data model, serialization, limits,
fixtures, registry). Exact wire prose must never remain in both files. The
resulting identity spec must be short enough to implement directly and exact
enough that no language-default behavior leaks into bytes.

### 7.1 Primitive encodings

- `u16_be(n)` is exactly two unsigned big-endian bytes, valid for
  `0 <= n <= 65,535`.
- `u32_be(n)` is exactly four unsigned big-endian bytes, valid for
  `0 <= n <= 4,294,967,295`.
...
### 7.2 Domain-separated identity

Every identity domain is a literal registered ASCII byte string ending in one
NUL byte and containing no earlier NUL. M0 registers only the real manifest
domain:
...
Fixture-local domains such as `test:a\0` and `test:b\0` exercise framing but are
not registered product domains. Future specifications append named domains; they
do not alter this framing.

For domain `D` and ordered byte fields `F`, the preimage is:

```text
D || u16_be(len(F)) || concat(u32_be(len(field)) || field for field in F)
```

The identity is SHA-256 of the preimage, rendered as exactly 64 lowercase
hexadecimal characters. There is no implicit UTF-8 conversion, Unicode
normalization, path normalization, newline normalization, delimiter, field
sorting, or terminal LF. A caller that starts with text must define its own
text-to-byte rule before invoking the identity primitive.

Manifest identity has exactly one field: the complete canonical-manifest bytes,
including their required final LF, under `golden-board:manifest:v0\0`.

### 7.3 Canonical manifest data model

One manifest is one top-level object. Nested values may be:

- objects;
- arrays;
- strings containing valid Unicode scalar values;
- booleans; and
- integers in `0..=18,446,744,073,709,551,615` (`u64`).

The following are outside v0 and reject: `null`, negative numbers, negative zero,
floats, exponent notation, arbitrary-precision integers, byte strings, dates,
comments, duplicate decoded keys, and trailing data.

Object keys are Unicode strings restricted after decoding to bytes in the ASCII
range `0x20..0x7e`, inclusive. Empty keys, controls, DEL, and non-ASCII keys
reject. Keys sort lexicographically by their decoded ASCII bytes. Source order
never controls output.

Strings preserve their exact Unicode scalar sequence. No NFC/NFD or other
normalization occurs. Invalid UTF-8 and unpaired UTF-16 surrogate escapes reject;
a valid surrogate pair decodes to its scalar value.

### 7.4 Canonical serialization

- emit UTF-8 with no BOM;
- emit object keys in raw decoded ASCII-byte order;
- emit arrays in input order;
- emit `true` and `false` in lowercase;
- emit integers as shortest ASCII decimal with no sign or leading zero, except
  zero is `0`;
- emit no insignificant whitespace;
- escape `"`, `\`, backspace, tab, LF, form feed, and CR as `\"`, `\\`, `\b`,
  `\t`, `\n`, `\f`, and `\r`;
- escape every other scalar `U+0000..U+001F` as lowercase `\u00xx`;
- emit `U+007F` raw as its single UTF-8 byte `7f`;
- do not escape `/`, printable ASCII, or non-ASCII scalar values;
- emit exactly one LF after the top-level object and no other trailing byte.

Canonical validation parses under the v0 data model, serializes again, and
requires byte-for-byte equality with the input. Thus otherwise valid JSON with
indentation, reordered keys, `\u0061` for `a`, escaped `/`, a BOM, or a missing
final LF is noncanonical and rejects.

### 7.5 Resource bounds

M0 manifests are intentionally small. Both implementations enforce:

| Resource | Limit |
|---|---:|
| Input or serialized output bytes, including final LF | 1,048,576 |
| Nesting depth, counting top object as 1 | 32 |
| Integer | `u64::MAX` |

Key, string, and container sizes need no separate arbitrary ceiling because the
complete input-byte bound already limits them. The parser must not recurse or
allocate based on an unchecked length. These are developer-manifest v0 limits,
not transport profile limits.

Only objects and arrays increase depth; scalar children occupy their containing
depth. The top-level object is depth 1, so its directly nested object/array is
depth 2. Parsing rejects before descending to depth 33. Serialization counts
bytes as it emits and rejects rather than returning output above 1,048,576 bytes.

### 7.6 Conformance fixtures

`conformance/identity-v0.json` is a canonical manifest containing hand-authored
vectors for:

- raw SHA-256 empty and one-byte `d3` known answers from NIST CAVS 11.0
  `SHA256ShortMsg.rsp`;
- zero framed fields;
- one empty field, proving it differs from zero fields;
- `a,bc` versus `ab,c`, proving field-boundary separation;
- identical fields under two fixture-local test domains, proving separation;
- a binary field containing `00`, `7f`, `80`, and `ff`;
- field-count boundary helpers; and
- lowercase-hex output.

`conformance/manifest-v0.json` stores fixture payloads as lowercase hex so one
canonical fixture can describe non-UTF-8 and deliberately noncanonical inputs.
It covers at least:

- empty and nested objects/arrays;
- reversed keys and prefix-related keys;
- quote, backslash, slash, every short control escape, and a `\u00xx` control;
- raw `U+007F` output and escaped `\u007f` noncanonical input;
- raw non-ASCII scalars and valid surrogate-pair decoding;
- canonically distinct NFC and NFD strings;
- `0` and `u64::MAX`;
- duplicate decoded keys, including literal/escaped aliases;
- BOM, invalid UTF-8, unpaired surrogates, non-ASCII keys, and control keys;
- `null`, negative, negative zero, float, exponent, integer overflow, and leading
  zero;
- comments and trailing data;
- whitespace, key-order, escape, and final-LF noncanonical forms;
- exact depth limit and one-over-limit rejection; and
- exact input and serialized-output size limits plus one-over-limit rejection,
  without committing huge fixture files (tests construct boundaries
  deterministically).

Expected identity digests and canonical bytes are authored from the specification
and independently checked before commit. Tests never rewrite expected fixtures.
Regeneration, if later added as an authoring aid, writes only to `artifacts/` and
requires human comparison.

### 7.7 Registry

`conformance/registry.toml` contains only the two existing fixture suites. Each
entry has a unique ID, path, specification/version, payload SHA-256, consumers
`python` and `rust`, and provenance `hand-authored`. Paths must exist, hashes must
match, and files under `conformance/` other than the registry itself must be
registered or the repo check fails.

There are no empty arrays or placeholder slots for chess, source, transport,
curriculum, damage, or browser vectors. Their owning milestones add real entries.
