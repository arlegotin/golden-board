...
- [x] Run the full command and record actual report/fixture/registry identities
...
- [x] Trace each roadmap M0 exit criterion and M0 spec Section 14 row to an
...
- [x] Confirm M0 spec Section 10 still records the verified Docker 25.0.3
...
- [x] Review the complete diff for:
...
- [x] Audit `scripts/check full` coverage for the high-risk cases already built
...
- [x] Check the repository remains intentionally small:
...
- [x] Create the cold-start candidate commit while M0 still says `In progress`.
