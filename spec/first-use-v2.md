...
adds numeric opcode bounds/types and packed-tag examples. Fact3 carries the
canonical ULEB integer examples and fact5 distinguishes fixed route descriptors
from compact recipe descriptors. Those together frame symbolic dataflow and
...
* EMIT has exact input-copy anchors in211: both inputs1/2 into output33 at
  ...
  positions come from carried descriptors and actual variable-length node fields.
...
Parse the seven observed fact5 L tables in order. Each layout row is
`[layout_id, definition_offset, [[field_offset,field_width],...]]`, IDs0..6
for route envelope, frame, package, table, recipe, route descriptor, compact
recipe descriptor. Definition
...
zeros and counts. Layouts0..5 partition their fixed headers without gaps.
Layout6 is exactly `[(0,1),(1,0)]`: type is one byte and the zero width marks
the canonical ULEB32 width field whose actual span is derived from its bytes.
...
The encoding2 package is partitioned exactly into its64-byte header, table
headers and payloads, recipe32-byte headers,2..6-byte compact descriptors and
variable-length nodes under recipe-wire-v2. Every ULEB is canonical and bounded
by both its integer type and its containing recipe. Expanded logical byte
positions are never substituted for observed compact positions.
...
item0), recipe headers(owner=recipeID,item0), and compact descriptors(layout6,
owner=recipeID,item=input index or65536+output index). All rows follow wire order.
Compact descriptors contribute exactly two actual fields: type and the complete
ULEB width span. No descriptor ID, flags or count field has a wire position.

`node_field_rows` are `[recipe,node_index,kind,ordinal,offset,length]`, in
wire order. Kinds0/1/2/3/4 mean tag/width/argument/auxiliary/immediate.
Ordinal is one-based for arguments and zero otherwise. The tag is one byte,
with opcode in low five bits and output type in high three bits. Width is
ULEB32, arguments and auxiliary are ULEB16, immediate is ULEB64. Every node's
field spans partition exactly its actual bytes. The actual fact6 row supplies
arity and maximum field/node lengths, not fixed lengths. Optional auxiliary
and immediate fields exist only for the carried opcode. A maximum is never
recorded as an actual field width. Implicit node IDs are input_count+node_index
and are not fictional wire bytes.
...
1=output, index one-based. ID equals that ordinal, count is1 and flags are0
by the compact grammar; these are explicit logical derivations, not carried
descriptor fields. Start names the actual type byte. Its length is the sum of
the two corresponding layout6 field spans. `node_rows`:
...
the producer is its actual compact input descriptor or a preceding node of this recipe.
...
`[field_offset,value]`; its actual canonical ULEB span is recorded in
`node_field_rows`, as are each argument and auxiliary field's encoded widths.
The immediate is the explicit constant/offset/count, whose type and role come
from that node and fact6. No host constant substitutes.
...
30/109/113/120/123/127/202, and table17's explicit fact9 mapping lookup. Every package
...
After fact10's framed roots, add its observed embedded composition root at
DEFINE offset8 (recipe126); observed admission executes all eight57-byte rows
beginning at22. Each row contains query U32, case U8 and52 observed result bytes.
This is an explicit carried use, not a claim of an ordinary framed example.
Fact10's first20 bytes name123/124/120/125/126/127 and tables24/25/26/27.
Closure enumeration follows actual TABLE and ITERATE references; those header
names alone do not create fabricated execution edges or examples.
...
Operational rows use fact0, ordered30/109/113/120/123/127/202, after example rows. Fact0
denotes receiver use after primitive grounding, never an extra defining fact.
The profile8 observation receiver dispatches127 for the first inventory group,
120 for subsequent inventory and admitted groups, and123 for physical lookup
after inventory admission. These are actual operational uses with the full
observed VM charge; fact10 header references and source-generated transfer tests
alone would not establish them. Transfer tests are not carried ordinary examples.
Every recipe must remain in an actual example or receiver-use closure; linkage
alone cannot replace the executed-use requirement.
...
`node_rows`, `node_field_rows`, `literal_rows`, `copy_rows`, `opcode_rows`, `type_rows`,
